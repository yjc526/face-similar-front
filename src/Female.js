...
export default function Female({ result }) {
  ...
  const emo1 = result.info.female.emotion.first.emotion;
  const emo1Num = result.info.female.emotion.first.score;
  const emo1Number = Math.round(emo1Num * 100 * 100) / 100;
  const emo2 = result.info.female.emotion.second.emotion;
  const emo2Num = result.info.female.emotion.second.score;
